...
## PRE_WRITE — 와이어프레임 라벨·블루프린트 작성 직전 룰

**목적:** 화면 라벨·버튼명·블루프린트 본문 *작성 시점*에 도메인 어휘·단문·결과형 강제. UX_CHECKLIST 사후검증 + PRE_WRITE 사전생성 = 이중 게이트.

### 5룰 (작성 직전 강제)

| # | 룰 | UI 결로 변환 | FAIL 신호 |
|---|---|---|---|
| 1 | **버튼·라벨 단문** | 버튼 ≤6자, 탭라벨 ≤4자, 헤더 ≤12자 | "지금 바로 시작하기" 길게 |
| 2 | **단일 행동** | 1버튼 = 1행동. 묶기 ✗ | "구매하고 공유하기" |
| 3 | **도메인 어휘** | Lorem·Tab1·John Doe ✗. 도메인 실제 어휘 | 일반어·placeholder |
| 4 | **결과형 라벨** | 버튼 = 사용자 액션 결과 (담기·결제·예약) | "제출"·"확인" 추상 |
| 5 | **AI식 사전회피** | "사용자 친화적인", "직관적인 인터페이스", "원활한 경험" — 블루프린트 본문 차단 | UX 관용어 |

### 단계별 PRE_WRITE

- **③ 블루프린트**: 4축 정의·진보 설계 포인트 = 단언형 1줄 ≤25자
- **③-b UX_CHECKLIST**: PRE_WRITE 5룰 통과 후 N2·N4·N6·D1·D3 검증 (이중 게이트)
- **④ 와이어 HTML**: 모든 라벨·버튼 = 5룰 강제. Lorem 차단 = PRE_WRITE Rule 3

### 자체검증

| # | 체크 | 위반 |
|---|------|------|
| 1 | 버튼 ≤6자, 결과형? | 초과·추상 = 재작성 |
| 2 | 도메인 어휘? | Lorem·일반어 = 도메인 변환 |
...
| ALLOW | 업계 전문용어(UX·UI·CTA·PRD·MVP·CRUD·API·SDK) · 고유명사(앱명·플랫폼명) · 법조문 |
| CONVERT | 라벨 발견 → 실명·평문 풀어쓰기. 예) "28도메인×190+앱 패턴DB" → 실제 도메인·앱명 / "TURBO·SHE·UX_CHECKLIST" → 결과만 노출 |
| SELF_CHECK | 와이어프레임 HTML 출력 직전에서 자체 스캔. 1개라도 발견 = 차단·재작성. paper-engine cascade 경유 시 INV 13 자동 적용 |
